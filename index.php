...
function validateAndFormatProxy() {
    ...
    // Check if proxy is in IP:PORT format
    if (preg_match('/^(\d+\.\d+\.\d+\.\d+):(\d+)$/', $proxy)) {
        return $proxy;
    }
    
    // Check if proxy is in IP:PORT:USERNAME:PASSWORD format
    if (preg_match('/^(\d+\.\d+\.\d+\.\d+):(\d+):([^:]+):(.+)$/', $proxy)) {
        return $proxy;
    }
...
if ($proxy) {
    ...
    // If proxy has authentication (format: IP:PORT:USERNAME:PASSWORD)
    if (strpos($proxy, ':') !== false && substr_count($proxy, ':') >= 3) {
